...
- [x] Day 1-2: Thiết kế Schema `Product` & `ProductSKU` (Biến thể kèm giá/kho riêng).
- [x] Day 3-4: API Seller: `POST /api/v1/products` (Tạo sản phẩm & SKU).
- [x] Day 5: API Seller: Quản lý danh sách và trạng thái ẩn/hiện sản phẩm.
- [x] Day 6: Media: Tích hợp Upload ảnh sản phẩm (Cloudinary/S3).
- [x] Day 7: Validation: Ràng buộc logic Shop - Product - Category.

#### **Sprint 5 (Week 5): Smart Cart & Marketplace Checkout**
*Mục tiêu tuần: Giỏ hàng hỗ trợ nhiều shop và xử lý trừ kho an toàn.*
- [x] Day 1-2: Module Cart: Lưu trữ giỏ hàng, tính toán giá real-time từ SKU.
- [x] Day 3: API Checkout Preview: Gom nhóm sản phẩm theo Shop & Tính phí ship.
- [x] Day 4-5: API `POST /api/v1/orders`: Xử lý trừ kho an toàn (Pessimistic Locking).
