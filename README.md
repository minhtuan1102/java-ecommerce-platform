...
- [ ] Day 1: Tạo `SecurityConfig` với public/private endpoint.
- [ ] Day 2: Implement `POST /api/v1/auth/register` (BCrypt password).
- [ ] Day 3: Implement `POST /api/v1/auth/login` (Trả JWT access token).
- [ ] Day 4: JWT middleware (Filter đọc Authorization header, Verify token).
- [ ] Day 5: Bảo vệ endpoint Category bằng JWT theo policy.
- [ ] Day 6: Viết test cho register/login.
- [ ] Day 7: Dọn code, xử lý error format chung, chuẩn bị vào Marketplace.
...
- [ ] Day 1-2: Thiết kế Database cho `Shop` (name, owner_id, status) & Flyway migration.
- [ ] Day 3: API `POST /api/v1/shops` (Đăng ký shop & Tự động cập nhật `ROLE_SELLER`).
- [ ] Day 4: API `GET /api/v1/shops/my-shop` (Quản lý thông tin gian hàng).
- [ ] Day 5: Security: Cấu hình phân quyền truy cập endpoint Seller.
- [ ] Day 6-7: Viết Integration Test cho luồng đăng ký shop.
